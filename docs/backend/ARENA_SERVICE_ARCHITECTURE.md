...
Phase 4 adds `src/server/submissions/` for ownership-scoped drafts, deadline checks, immutable version snapshots, transactional review-attempt allocation, and SSRF-safe link checks. `src/server/storage/` is the server-only private-R2 boundary: it creates random keys and short-lived presigned URLs, then validates object metadata after upload. Route handlers never accept browser identity, storage credentials, or client-chosen keys.
