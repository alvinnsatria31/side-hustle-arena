...
#   STORE=true deploy/sk-vps/build-and-ship.sh      # bake the digital shop on
...
# The shop's switch is read by client code, so it is baked in. Midtrans keys are
# runtime variables in arena.env and deliberately absent here.
STORE="${STORE:-}"
