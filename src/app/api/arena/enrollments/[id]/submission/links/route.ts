...
import { addArenaSubmissionLink } from "@/server/submissions";
...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    return arenaData(await addArenaSubmissionLink({ userId: user.id, enrollmentId: (await params).id, input: await request.json() }), 201);
  } catch (error) {
