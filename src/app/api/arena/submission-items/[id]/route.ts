...
import { arenaData, arenaError, arenaUnauthorized, enrollmentIdSchema } from "@/server/arena";
...
export async function GET(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!enrollmentId || !enrollmentIdSchema.safeParse(enrollmentId).success) {
    return arenaError(new ArenaDomainError("VALIDATION_ERROR", "Enrollment id is required."));
  }
  try {
