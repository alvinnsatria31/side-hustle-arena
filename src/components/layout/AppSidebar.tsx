...
export function AppSidebar() {
  ...
  return (
    <aside className="sticky top-0 hidden h-screen w-[244px] shrink-0 flex-col bg-[#102544] px-4 py-6 text-white lg:flex xl:w-[260px]">
      ...
      <p className="mb-3 mt-11 px-3 font-mono text-[10px] font-bold uppercase tracking-[0.17em] text-[#7d9abf]">Ruang kerja</p>
        ...
        {appNavLinks().map((link) => {
          ...
          return (
            <Link
              ...
              className={cn(
                'flex min-h-11 items-center gap-3 rounded-xl px-3 text-[13px] font-semibold text-[#b9cbe3] transition-colors hover:bg-white/10 hover:text-white focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-white',
                active && 'bg-[#2a6bea] text-white shadow-[0_8px_22px_rgba(12,69,157,0.28)] hover:bg-[#2a6bea]',
              )}
            >
              <Icon size={18} strokeWidth={active ? 2.3 : 2} aria-hidden />
              {link.label}
            </Link>
          );
        })}
      ...
      <div className="mt-auto space-y-1 border-t border-white/10 pt-5">
        <Link href="/app/profile" className="flex min-h-11 items-center gap-3 rounded-xl px-3 text-[13px] font-semibold text-[#b9cbe3] transition-colors hover:bg-white/10 hover:text-white">
          <UserRound size={18} aria-hidden /> Profil
        </Link>
        <a href={WHATSAPP_SUPPORT_URL} target="_blank" rel="noopener noreferrer" className="flex min-h-11 items-center gap-3 rounded-xl px-3 text-[13px] font-semibold text-[#b9cbe3] transition-colors hover:bg-white/10 hover:text-white">
