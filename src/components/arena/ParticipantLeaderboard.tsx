...
export default function ParticipantLeaderboard() {
  ...
  return <ParticipantShell title="Peringkat" action={<RefreshButton refresh={refresh} loading={leaderboard.loading || overview.loading} />}>
    <div className="mb-6 flex flex-wrap items-center justify-between gap-4">
      <label className="flex max-w-full flex-wrap items-center gap-3 text-sm font-semibold text-sk-navy">Minggu
        <select aria-label="Minggu peringkat" value={week} onChange={(event) => setWeek(event.target.value)} className="h-11 w-64 max-w-full rounded-md border border-sk-border bg-white px-3 text-sm font-normal">
    ...
    {unpublished && !leaderboard.loading && <p role="status" className="border-y border-sk-border py-8 text-sm text-sk-muted">{week ? 'Peringkat minggu ini belum diumumkan.' : 'Belum ada peringkat yang diumumkan.'}</p>}
    {leaderboard.data && !leaderboard.loading && <section>
      ...
      {leaderboard.data.rows.length === 0 ? <p className="py-8 text-sm text-sk-muted">Belum ada peserta yang masuk peringkat minggu ini.</p> : <div className="overflow-x-auto border-y border-sk-border">
        <table className="w-full min-w-[540px] text-left text-sm">
          <caption className="sr-only">Peringkat {leaderboard.data.weekCode}</caption>
          <thead className="bg-sk-bg text-xs text-sk-muted"><tr><th scope="col" className="p-4">Peringkat</th><th scope="col" className="p-4">Peserta / Proyek</th><th scope="col" className="p-4 text-right">Skor</th><th scope="col" className="p-4 text-right">Poin</th></tr></thead>
