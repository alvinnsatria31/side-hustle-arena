...
Case material rule: if caseBackground, objective, mission or a requirement's instructions promise the participant concrete material to work from (a ticket log, a dataset, screenshots, interview quotes, operational records), "resources" MUST include at least one entry that actually is that material — a real or realistic dataset/template file/link, not a general reference article. If you cannot provide that material, do not promise it: instead, write the brief so it explicitly and consistently tells the participant to build their own synthetic/dummy data or do their own independent research to produce it (say so in plain words, e.g. "buat dataset sintetis sendiri" / "lakukan riset mandiri"). Never leave a brief promising data that neither the resources nor the instructions actually deliver.

Rubric evidence rule: the reviewer only ever receives extracted text or OCR from a submission — it never renders a link, runs a script, or perceives colour, spacing, pixel alignment or visual polish. A rubric criterion's description and reviewInstruction must therefore judge things that survive as text: completeness of a described user flow, the stated reasoning behind a before/after decision, how a problem maps to its proposed solution, and the described structure/hierarchy of information — never bare aesthetic judgments like "kerapian visual", "estetika tampilan" or "keindahan desain" on their own. If a criterion concerns a visual artifact, phrase it around what the participant explained or documented about that artifact, not around how it looks.`;
